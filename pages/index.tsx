...
function CameraController({ currentHash, isCinematic }: { currentHash: string, isCinematic: boolean }) {
  ...
  useEffect(() => {
    ...
    const onEndDrag = () => { isUserDragging.current = false; };
  ...
  useFrame((state) => {
    ...
    if (currentHash !== "#overview" && ORBIT_CONFIG[currentHash]) {
      ...
      // Điểm nhìn (Target) nằm giữa hành tinh và UFO một chút để thấy cả hai
      targetCenter.set(planetX + 5, 2, planetZ + 5);
      
      // Vị trí Camera: Đẩy ra xa hơn offset của hành tinh một chút để lấy góc rộng
      targetCamPos.set(
    ...
    // Cập nhật tâm nhìn của OrbitControls
    controls.target.lerp(targetCenter, 0.05);
    
    // Chỉ tự động di chuyển camera nếu người dùng không đang xoay chuột
