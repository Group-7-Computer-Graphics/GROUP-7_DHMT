...
const BackgroundEffects = () => {
  ...
  useFrame(state => {
    ...
    shootingStarRef.current?.children.forEach(star => {
      star.position.x += star.userData.velX
      star.position.y += star.userData.velY

      star.material.opacity =
        0.6 +
        Math.abs(Math.sin(state.clock.elapsedTime * 3 + star.userData.angle)) *
          0.4

      if (
        Math.abs(star.position.x) > 1500 ||
        Math.abs(star.position.y) > 1500
      ) {
